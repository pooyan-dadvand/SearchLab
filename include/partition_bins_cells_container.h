...
#include "mpi.h"
#include "distributed_bounding_box.h"

class PartitionBinsCellsContainer {
...
protected:
	...
	DistributedBoundingBox<InternalPointType> mBoundingBox;
...
public:
	...
	PartitionBinsCellsContainer(TIteratorType const& PointsBegin, TIteratorType const& PointsEnd, std::size_t approximated_number_of_cells)
...
protected:
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &mpi_size);
		...
		for (auto i_point = PointsBegin; i_point != PointsEnd; i_point++) {
			mCellsBeginIndices[CalculateCellIndex(*i_point) + 1] |= 1;
		}
