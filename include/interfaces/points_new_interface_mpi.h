...
namespace PointsNew {
  ...
  template < class BinsType >
    ...
    std::size_t pointOffset =
      ( mpi_rank < numBigProc ) * ( mpi_rank * sizeBigProc ) +
      ( mpi_rank >= numBigProc ) *
      ( ( numBigProc * sizeBigProc ) + ( ( mpi_rank - numBigProc ) * sizeLowProc ) );
    ...
    std::vector< std::vector< PointsBins< Point >::ResultType > > resultsArray(
        numProcPoints, std::vector< PointsBins< Point >::ResultType >( 0 ) );
